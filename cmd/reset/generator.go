...
// methodData is the template data for a single generated Reset() method.
type methodData struct {
	Recv     string
	TypeName string
	// Stmts are the method body's statements, already indented one level
	// (interior newlines of a multi-line statement, e.g. a nil-guarded
	// pointer reset, are indented too — see indent()). The template places
	// them as-is between the nil check and the closing brace.
	Stmts []string
}

// generateReset computes the template data for the Reset() method of a
// struct named typeName with fields structType. resettable is the set of
// type objects that either already have a Reset() method or will get one
// generated in this run.
func generateReset(typeName string, structType *types.Struct, resettable map[types.Object]bool) methodData {
	...
	return methodData{Recv: recv, TypeName: typeName, Stmts: stmts}
}
...
// fileData is the template data for one package's reset.gen.go.
type fileData struct {
	Package string
	Methods []methodData
}

// resetFileTemplate renders a package's reset.gen.go. format.Source cleans
// up indentation and 2+ blank lines afterwards, but the "-}}" trims around
// the per-statement loop are still needed: without them, gofmt leaves a
// lone blank line between every statement.
var resetFileTemplate = template.Must(template.New("file").Parse(
...
func writeResetFile(pkg *packages.Package, methods []methodData) error {
	...
	data := fileData{Package: pkg.Name, Methods: methods}
	if err := resetFileTemplate.Execute(&buf, data); err != nil {
